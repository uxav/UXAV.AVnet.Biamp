...
namespace UXAV.AVnet.Biamp.ControlBlocks
{
    ...
    public class DialerBlock : TesiraBlockBase, IEnumerable<DialerLine>
    {
        ...
        public override TesiraBlockType Type => TesiraBlockType.DialerBlock;

        public int NumberOfLines => _channels.Count;

        public DialerLine this[uint line] => _channels[line];
                ...
                switch (response.AttributeCode)
                {
                    case TesiraAttributeCode.NumChannels:
                        ...
                        for (uint i = 1; i <= numberOfChannels; i++)
                        {
                            ...
                            {
                                Logger.Debug($"Creating Dialer Line {i}");
